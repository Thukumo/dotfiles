...
let
  ...
  unpacked = stdenvNoCC.mkDerivation {
    ...
    installPhase = ''
      ...
      # MPRIS integration only sets xesam:url when the item has a share URL
      # attribute; patch the bundled app.asar to fall back to getShareUrl()
      python3 ${./asar-patch.py} $out/opt/resources/app.asar
